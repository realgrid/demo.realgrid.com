...
Auto Filter버튼 클릭 시 `CustomerID` 컬럼에 `Auto Filter`가 설정되며
Auto Filter의 textBox에 `value` 값 입력 후 Enter키 입력 시 필터가 추가됩니다.  
Auto Filter의 textBox에 'value' 값을 입력하면 해당 컬럼의 데이터를 기준으로 자동완성 기능이 활성화됩니다.  
해당 filter값을 체크 후 `Apply버튼`을 클릭하면 선택된 값으로 필터링 합니다.

```js
//자동완성 기능을 위한 jquery-ui, css 
...
<a class="btn primary small round lowercase" id="btnSetFilters">Auto Filter</a>
